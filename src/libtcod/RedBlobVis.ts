// translated from http://www.adammil.net/blog/v125_Roguelike_Vision_Algorithms.html#mycode
...
class Slope {
	...
	gt(y: number, x: number) {
		return this.y * x > this.x * y;
	}
	gte(y: number, x: number) {
		return this.y * x >= this.x * y;
	}
	lt(y: number, x: number) {
		return this.y * x < this.x * y;
	}
}
...
export default class RedBlobVis {
	...
	private computeOctant(
		...
		for (
			;
			x <= rangeLimit;
			x++ // (x <= (uint)rangeLimit) == (rangeLimit < 0 || x <= rangeLimit)
		) {
			...
			for (
				...
				if (
					...
					var isVisible =
						isOpaque ||
						((y != topY || top.gt(y * 4 - 1, x * 4 + 1)) &&
							(y != bottomY || bottom.lt(y * 4 + 1, x * 4 - 1)));
